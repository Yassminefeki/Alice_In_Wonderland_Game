...
const float monster_speed = fullmovement * 0.75;
const float monster_fright = fullmovement * 0.5;
...
struct monster
{
	...
	float move_speed = monster_speed;
...
struct GameState
{
	...
	bool player_eat_monster = false;
	int monsters_eaten_in_powerup = 0;
	monster* recent_eaten = nullptr;
...
inline bool monsterRetreating()
{
	for (int i = 0; i < 4; i++) {
		if (gState.monsters[i]->target_state == GOHOME)
			return true;
	}
